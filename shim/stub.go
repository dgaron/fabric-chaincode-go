...
func (s *ChaincodeStub) GetVersionForKey(key string, start uint64, end uint64) (HistoryQueryIteratorInterface, error) {
	response, err := s.handler.handleGetVersionForKey(key, start, end, s.ChannelID, s.TxID)
	if err != nil {
		return nil, err
	}
	return &HistoryQueryIterator{CommonIterator: &CommonIterator{s.handler, s.ChannelID, s.TxID, response, 0}}, nil
